...
namespace appweeb
{
    ...
    private:
        ...
        static void SendResponse( Socket& client, int statusCode, std::string_view contentType, const void* data, size_t size);
        void HandleClient(Socket client);
        std::string ReceiveRequest(Socket& client);
        static void SendJsonResponse(Socket& client, bool success, std::string_view error = {});
        std::string ReadTextFile(const std::filesystem::path& path);
        bool WriteBinaryFile(const std::filesystem::path& path, const void* data, size_t size);
        std::string GetHeaderValue(std::string_view request, std::string_view headerName);
    private:
        uint16_t m_port;
        Socket m_listener;
        bool m_running = true;
